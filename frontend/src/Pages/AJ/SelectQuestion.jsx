import React from 'react'
import { Link } from 'react-router-dom'
import Navbar from '../../Components/Navbar'

const SelectQuestion = () => {
  return (
    <div>
        <Navbar/>

        <div className='flex max-w-[1100px] justify-between mx-auto mt-[50px]  text-white'>
            <h2 className='flex flex-col font-medium text-3xl'>
                Select Questions
            </h2>

            <h3 className='font-light flex text-xl'>
                logged in as : Dissanayake D M A P
            </h3>
        </div>

{/* Question 01 */}
        
        <div className='max-w-[1100px] justify-between mx-auto mt-[50px] bg-slate-800 py-2 px-2 text-white grid grid-cols-5 gap-4 '>
                <Link>
                <h2 className='flex flex-col font-normal text-xl'>Question 01 </h2>
                </Link>

                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>view</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>edit</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>delete</button>
                <button className='bg-slate-600 cursor-pointer  hover:bg-[#06C6D5] hover:text-black duration-500 px-[10px] py-1'>Add to Labsheet</button>
        </div>


        {/* Question 02 */}
        
        <div className='max-w-[1100px] justify-between mx-auto mt-[5px] bg-slate-800 py-2 px-2 text-white grid grid-cols-5 gap-4 '>
                <Link>
                <h2 className='flex flex-col font-normal text-xl'>Question 02 </h2>
                </Link>

                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>view</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>edit</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>delete</button>
                <button className='bg-slate-600 cursor-pointer  hover:bg-[#06C6D5] hover:text-black duration-500 px-[10px] py-1'>Add to Labsheet</button>
        </div>

        {/* Question 03 */}
        
        <div className='max-w-[1100px] justify-between mx-auto mt-[5px] bg-slate-800 py-2 px-2 text-white grid grid-cols-5 gap-4 '>
                <Link>
                <h2 className='flex flex-col font-normal text-xl'>Question 03 </h2>
                </Link>

                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>view</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>edit</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>delete</button>
                <button className='bg-slate-600 cursor-pointer  hover:bg-[#06C6D5] hover:text-black duration-500 px-[10px] py-1'>Add to Labsheet</button>
        </div>

        {/* Question 04 */}
        
        <div className='max-w-[1100px] justify-between mx-auto mt-[5px] bg-slate-800 py-2 px-2 text-white grid grid-cols-5 gap-4 '>
                <Link>
                <h2 className='flex flex-col font-normal text-xl'>Question 04 </h2>
                </Link>

                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>view</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>edit</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>delete</button>
                <button className='bg-slate-600 cursor-pointer  hover:bg-[#06C6D5] hover:text-black duration-500 px-[10px] py-1'>Add to Labsheet</button>
        </div>

        {/* Question 05 */}
        
        <div className='max-w-[1100px] justify-between mx-auto mt-[5px] bg-slate-800 py-2 px-2 text-white grid grid-cols-5 gap-4 '>
                <Link>
                <h2 className='flex flex-col font-normal text-xl'>Question 05 </h2>
                </Link>

                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>view</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>edit</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>delete</button>
                <button className='bg-slate-600 cursor-pointer  hover:bg-[#06C6D5] hover:text-black duration-500 px-[10px] py-1'>Add to Labsheet</button>
        </div>

        {/* Question 06 */}
        
        <div className='max-w-[1100px] justify-between mx-auto mt-[5px] bg-slate-800 py-2 px-2 text-white grid grid-cols-5 gap-4 '>
                <Link>
                <h2 className='flex flex-col font-normal text-xl'>Question 06 </h2>
                </Link>

                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>view</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>edit</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>delete</button>
                <button className='bg-slate-600 cursor-pointer  hover:bg-[#06C6D5] hover:text-black duration-500 px-[10px] py-1'>Add to Labsheet</button>
        </div>

        {/* Question 07 */}
        
        <div className='max-w-[1100px] justify-between mx-auto mt-[5px] bg-slate-800 py-2 px-2 text-white grid grid-cols-5 gap-4 '>
                <Link>
                <h2 className='flex flex-col font-normal text-xl'>Question 07 </h2>
                </Link>

                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>view</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>edit</button>
                <button className='bg-slate-600 cursor-pointer hover:scale-105 duration-500 px-[10px] py-1'>delete</button>
                <button className='bg-slate-600 cursor-pointer  hover:bg-[#06C6D5] hover:text-black duration-500 px-[10px] py-1'>Add to Labsheet</button>
        </div>

        {/* add Question */}

        <div className='flex flex-col max-w-[1100px] mx-auto items-center justify-center mt-[30px] text-white text-xl mb-[50px]'>
            <Link to=''>
                <button className= 'text-center bg-[#191919] px-[150px] hover:bg-[#06C6D5] hover:scale-105 duration-500 py-4'>Add Questions</button>
            </Link>     
        </div>
    </div>
  )
}

export default SelectQuestion